...
class Encoder(nn.Module):
    def __init__(self, L, N):
        ...
        self.L = L  # 卷积核大小
        self.N = N  # 输出通道大小
        self.Conv1d = nn.Conv1d(in_channels=1,out_channels=N,kernel_size=L,stride=L//2,padding=0,bias=False)
        self.ReLU = nn.ReLU()
    def forward(self, x):
        x = self.Conv1d(x)
        x = self.ReLU(x)
...
class Decoder(nn.Module):
    def __init__(self, L, N):
        ...
        self.L = L
        self.N = N
        self.ConvTranspose1d = nn.ConvTranspose1d(in_channels=N,out_channels=1,kernel_size=L,stride=L//2,padding=0,bias=False)
    def forward(self, x):
...
class DPTBlock(nn.Module):
    ...
    def forward(self, z):
        ...
        row_f = row_z1 + row_z
        row_output = row_f.view(B, P, K, N).permute(0, 3, 2, 1).contiguous()#B,N,K,P
        # inter DPT
...
class Sepformer(nn.Module):
    """
        Args:
            ...
            R: Number of repeats
    """
    def __init__(self, pertrain=False,pertrainpath=None,N=64, C=2, L=4, H=4, K=250, Global_B=2, Local_B=4):
        ...
        self.Local_B = Local_B  # 局部循环次数
        if pertrain:
            self.mode=torch.load(pertrainpath)
            self.encoder=self.mode.encoder
            self.decoder=self.mode.decoder
            self.masknet=self.mode.masknet
        else:
            self.encoder = Encoder(self.L, self.N)
            self.masknet = masknet(self.N, self.C, self.H, self.K, self.Global_B, self.Local_B)
    ...
    def forward(self, sig):
        ...
        x=torch.stack([x]*2)
        z=x*y
        audio=torch.cat([self.decoder(z[i]).unsqueeze(-1) for i in range(2)],dim=-1)
...
if __name__ == "__main__":
    ...
    model = Sepformer(N=128,C=2,L=2,H=8,K=250,Global_B=1,Local_B=1)
